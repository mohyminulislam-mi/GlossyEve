    ...
    const loadProduct = async () => {
      try {
        ...
        loadedColors.forEach(c => {
          if (STANDARD_COLORS.includes(c)) {
            initialSelectedColors.push(c);
          } else {
            initialCustomColors.push(c);
          }
        });
        setSelectedColors(initialSelectedColors);
        setCustomColors(initialCustomColors.join(", "));

        const loadedSizes = Array.isArray(product.sizes) ? product.sizes : [];
        const initialSelectedSizes = [];
        const initialCustomSizes = [];
        loadedSizes.forEach(s => {
          if (STANDARD_SIZES.includes(s)) {
            initialSelectedSizes.push(s);
          } else {
            initialCustomSizes.push(s);
          }
        });
        setSelectedSizes(initialSelectedSizes);
        setCustomSizes(initialCustomSizes.join(", "));
      } catch (error) {
        Swal.fire({
          icon: "error",
